...
it('isSuppressed returns false on NotFoundException', function () {
    $ctx = makeMockedSesClient([]);
    $ctx['calls'] = &$ctx['calls'];
    $client = $ctx['client'];

    // Replace queue with NotFoundException now that we have the client
    $reflection = new ReflectionClass($client);
    // simpler: append exception via a fresh mocked client
    $ctx = makeMockedSesClient([makeNotFoundException($client)]);
